...
1. Unprotected admin functionality
  (robots.txt) den admin panelini buldum ordan kullanıcıyı sildim.
...
2. Unprotected admin functionality with unpredictable URL
    ...
    (sayfanın kaynak kodlarında /admin-yhdjet admin panelin yerini bırakmışlar)
...
3. User role controlled by request parameter
    (admin panelinde sadece True yaparak bir kullanıcıyı sildim)
...
(e posta degiştirme yerinden  "roleid": 2 yaparak kendimi admin yetkisine çıkardım json veri)
...
(idyi carlos yaparak API keyini aldım)
...
(aynı şekil carlosun apu keyini aldım 5 teki gibi)
...
(id yi administrator yaptıktan sonra şifre sayfasından adminin şifresini bulup hesabına giriş yapıp carlosu silme)
